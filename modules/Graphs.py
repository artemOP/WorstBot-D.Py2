...
def pie(data: dict[str, int], *args, **kwargs) -> list[BytesIO]:
    ...
    """
    textprops = kwargs.get("textprops") or [
        {"color": "k", "size": "large", "weight": "heavy"},
        {"color": "w", "size": "large", "weight": "heavy"},
    ]
    plots = []
    for i in range(2):
        fig, ax = plt.subplots()
...
def bar(data: dict[str, int], *args, **kwargs) -> list[BytesIO]:
    ...
    """
    plt_props = kwargs.get("plt_props") or [
        {"label_colour": "black", "bg_colour": "white"},
        {"label_colour": "white", "bg_colour": "black"},
    ]
...
async def graph(
    graph_type: typing.Literal["pie", "bar"], loop: asyncio.AbstractEventLoop, data: dict[str, int], *args, **kwargs
) -> list[BytesIO] | None:
...
GRAPH_TYPES = {"pie": pie, "bar": bar, "line": line}
