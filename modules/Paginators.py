...
class ThemedGraphView(BaseView):
    ...
    @discord.ui.select(
        placeholder="Theme select", options=[discord.SelectOption(label="Light"), discord.SelectOption(label="Dark")]
    )
    async def Select(self, interaction: Interaction, select: discord.ui.Select):
        await interaction.response.edit_message(
            attachments=[discord.File(fp=self.graphs[select.values[0]], filename="image.png")]
        )
