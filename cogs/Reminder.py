...
class Reminder(commands.Cog):
    ...
    @app_commands.command(
        name="remindme", description="Set a DM reminder for all your important things (all fields are optional)"
    )
    @app_commands.guild_only()
    async def ReminderCreate(
        self,
        interaction: discord.Interaction,
        year: Range[int, dt.now().year, 2030] = None,
        month: Range[int, 1, 12] = None,
        day: Range[int, 1, 31] = None,
        hour: Range[int, 0, 60] = None,
        minute: Range[int, 0, 60] = 0,
        second: Range[int, 0, 60] = 0,
        message: str = "...",
    ):
        """Create a reminder
        ...
        await interaction.response.send_message(
            f"""Reminding you about "{message}" at {utils.format_dt(expiretime, style = "F")} ({utils.format_dt(expiretime, style = "R")})""",
            ephemeral=True,
        )
    ...
    async def ReminderTask(self):
        reminder = await self.bot.fetchrow(
            "SELECT * FROM reminder WHERE expiretime = (SELECT MIN(expiretime) FROM reminder)"
        )
