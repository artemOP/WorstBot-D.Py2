...
class Admin(commands.GroupCog, name="admin"):
    ...
    async def CogLoad(self, interaction: Interaction, cog: Choice[str]):
        ...
        await interaction.response.send_message(
            f"{cog.value} has been loaded", ephemeral=True
        )
    ...
    async def CogUnload(self, interaction: Interaction, cog: Choice[str]):
        ...
        await interaction.response.send_message(
            f"{cog.value} has been unloaded", ephemeral=True
        )
    ...
    async def CogReload(self, interaction: Interaction, cog: Choice[str]):
        try:
            ...
            await interaction.response.send_message(
                f"{cog.value} has been reloaded", ephemeral=True
            )
        except:
            await interaction.response.send_message(
                f"{cog.value} is not a valid input", ephemeral=True
            )
    ...
    async def nickname(self, interaction: Interaction, nickname: str = ""):
        ...
        await interaction.response.send_message(
            f"nickname set to {nickname}", ephemeral=True
        )

    @app_commands.command(
        name="profile", description="admin command to remove problematic profiles"
    )
    async def ProfileRemove(
        self, interaction: Interaction, user: discord.User, reason: str = None
    ):
        await self.bot.execute("DELETE FROM profile WHERE member = $1", user.id)
        await interaction.response.send_message(
            f"{user.name}'s profile has been removed"
        )
        try:
            await user.send(
                f"Your profile has been deleted by an administrator for reason:\n\n {reason}"
            )
        except discord.Forbidden:
            pass

    @app_commands.command(
        name="role", description="add or remove self assignable roles"
    )
    async def ToggleRole(self, interaction: Interaction, role: discord.Role):
        if not await self.bot.fetchval(
            "SELECT EXISTS(SELECT 1 FROM selfroles WHERE guild = $1 AND role = $2)",
            ...
            await interaction.response.send_message(
                f"{role.name} added as a self assignable role", ephemeral=True
            )
        else:
            await self.bot.execute("DELETE FROM selfroles WHERE role = $1", role.id)
            await interaction.response.send_message(
                f"{role.name} removed as a self assignable role", ephemeral=True
            )

    @app_commands.command(name="tags", description="delete all tags in the guild")
    async def DeleteAll(self, interaction: Interaction):
        await self.bot.execute(
            "DELETE FROM tags WHERE guild = $1", interaction.guild_id
        )
        await interaction.response.send_message(f"Deleted all tags", ephemeral=True)

    @app_commands.command(
        name="birthdays", description="Set or remove Birthday alerts channel"
    )
    async def BirthdayChannel(
        self, interaction: Interaction, channel: discord.TextChannel = None
    ):
        if not channel:
            await self.bot.execute(
                "DELETE FROM birthdaychannel WHERE guild = $1", interaction.guild_id
            )
            return await interaction.response.send_message(
                "Birthday channel has been removed", ephemeral=True
            )
        await self.bot.execute(
            "INSERT INTO birthdaychannel(guild, channel) VALUES($1, $2)",
            interaction.guild_id,
            channel.id,
        )
        await interaction.response.send_message(
            f"Birthday channel is now {channel.mention}", ephemeral=True
        )
