    ...
    async def on_submit(self, interaction: Interaction, /) -> None:
        if self.todo_id:
            await interaction.client.execute(
                "UPDATE todo SET name=$1, content=$2 WHERE todo_id = $3",
                self.todo_name.value,
                self.todo_content.value,
                self.todo_id,
            )
        else:
            self.todo_id = await interaction.client.execute(
                "INSERT INTO todo(guild, channel, owner, message, name, content, status) VALUES($1, $2, $3, $4, $5, $6, 'on_going') RETURNING todo_id",
...
class SetDeadline(ui.Modal):
    ...
    async def on_submit(self, interaction: Interaction, /) -> None:
        ...
        await interaction.client.execute(
            "UPDATE todo SET deadline=$1, status=$2 WHERE todo_id=$3",
            self.todo.deadline,
            self.todo.status.value,
            self.todo.id_,
        )
...
class TodoMenu(Paginators.BaseView):
    ...
    async def previous_todo(self, interaction: Interaction):
        row = await interaction.client.fetchrow(
            "SELECT todo_id, message, name, content, deadline, status, locked, urgent, silent FROM todo WHERE todo_id<$1 AND owner=$2 AND status!='cancelled' ORDER BY todo_id DESC LIMIT 1",
            self.todo.id_,
            interaction.user.id,
        )
    ...
    async def next_todo(self, interaction: Interaction):
        row = await interaction.client.fetchrow(
            "SELECT todo_id, message, name, content, deadline, status, locked, urgent, silent FROM todo WHERE todo_id>$1 AND owner=$2 AND status!='cancelled' ORDER BY todo_id LIMIT 1",
            self.todo.id_,
            interaction.user.id,
        )
        await self.form_todo(interaction, row)
    ...
    async def edit_todo(self, interaction: Interaction, button: ui.Button):
        ...
        await interaction.response.send_modal(
            CreateTodo(
                title="Edit TODO",
                todo_id=self.todo.id_,
                message=self.todo.message,
                todo_name=self.todo.name,
                todo_content=self.todo.content,
            )
        )
...
class SetFlags(Paginators.BaseView):
    ...
    async def update_flag(self, interaction: Interaction, button: ui.Button):
        ...
        await interaction.client.execute(
            "UPDATE todo SET locked=$2, urgent=$3, silent=$4 WHERE todo_id=$1",
            self.todo.id_,
            *asdict(self.todo.flags).values(),
        )
...
class TodoList(Paginators.ButtonPaginatedEmbeds):
    def __init__(self, embed_list: list[EmbedGen.Embed]):
        ...
        self.open_todo.options = [
            discord.SelectOption(label=todo.name, value=index)
            for index, todo in self.embedlist[self.page].extras.items()
        ]
...
class Todos(commands.GroupCog, name="todo"):
    ...
    async def cog_load(self) -> None:
        if not await self.bot.execute("SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'todo_status')"):
            await self.bot.execute(
                f"""CREATE TYPE todo_status as ENUM ({', '.join(f"'{status}'" for status in TodoStatus.__members__.values())})"""
            )
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS todo(todo_id SERIAL PRIMARY KEY, guild BIGINT, channel BIGINT, owner BIGINT, message BIGINT, name TEXT, content TEXT, status todo_status, deadline timestamptz, locked bool DEFAULT FALSE, urgent bool DEFAULT FALSE, silent bool DEFAULT FALSE, view_message BIGINT)"
        )
        self.context_menu = app_commands.ContextMenu(
            name="Add to TODO list", callback=self.todo_add_context, type=discord.AppCommandType.message
        )
    ...
    async def prepare_views(self):
        todo_rows = await self.bot.fetch(
            "SELECT todo_id, owner FROM todo WHERE status='expired' OR (status='overdue' AND view_message IS NOT NULL)"
        )
    ...
    async def get_todo(self, owner: discord.User, todo_id: int) -> Optional[Todo]:
        ...
        if todo := self.todos[owner.id].get(todo_id):
            return todo
        row = await self.bot.fetchrow(
            "SELECT todo_id, guild, channel, owner, message, name, content, status, deadline, locked, urgent, silent, view_message FROM todo WHERE owner=$1 AND todo_id=$2",
            owner.id,
            todo_id,
        )
    ...
    async def todo_add_context(self, interaction: Interaction, message: discord.Message):
        ...
        await interaction.response.send_modal(
            CreateTodo(
                title="Create TODO", message=message, todo_name=str(message.author), todo_content=message.content
            )
        )
    ...
    async def todo_list(self, interaction: Interaction, status: Optional[TodoStatus] = None):
        ...
        if status:
            todos = await self.bot.fetch(
                "SELECT todo_id FROM todo WHERE owner=$1 AND status=$2", interaction.user.id, status.value
            )
        else:
        ...
        embed_list = EmbedGen.EmbedFieldList(
            author={"name": interaction.user.display_name, "icon_url": interaction.user.display_avatar},
            title="TODOs",
            fields=[
                EmbedGen.EmbedField(
                    name=todo.name,
    ...
    async def todo_autcomplete(self, interaction: Interaction, current: Optional[str]):
        todos = await self.bot.fetch(
            "SELECT todo_id, name FROM todo WHERE owner=$1 AND status='on_going'", interaction.user.id
        )
        if not current:
    ...
    async def ReminderTask(self):
        row = await self.bot.fetchrow(
            "SELECT todo_id, owner FROM todo WHERE status IN ('on_going', 'overdue') AND deadline IS NOT NULL AND silent is FALSE ORDER BY deadline LIMIT 1"
        )
        if not row:
            return
        ...
        await self.bot.execute(
            "UPDATE todo SET status=$1, deadline=$2, view_message=$3 WHERE todo_id=$4",
            todo.status,
            todo.deadline,
            message.id,
            todo.id_,
