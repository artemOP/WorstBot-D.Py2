...
class CustomSearch(Transformer):
    ...
    async def transform(cls, interaction: Interaction, value: str, /) -> Playable | Playlist:
        ...
        elif platform is any(
            (SearchType.Spotify, SearchType.YouTubePlaylist, SearchType.Generic)
        ) and not value.startswith("https://"):
            raise app_commands.TransformerError(
                "A URL is required for this search type", discord.AppCommandOptionType.string, cls
            )
        else:
...
class Lavalink(commands.GroupCog, name="music"):
    ...
    async def join_vc(self, interaction: Interaction):
        ...
        if player is False:
            return await interaction.followup.send(
                "Please connect to a voice channel before using music commands", ephemeral=True
            )
        elif player is True:
        ...
        if player.current:
            ...
            return await player.channel.send(
                embed=EmbedGen.FullEmbed(
                    ...
                    fields=[
                        EmbedGen.EmbedField(
                            name="Duration", value=timedelta(milliseconds=getattr(search, "duration", 0))
                        ),
                        EmbedGen.EmbedField(name="Position", value=len(player.queue) + 1),
                        EmbedGen.EmbedField(
                            name="Estimated time until playing",
                            value=format_dt(utcnow() + timedelta(milliseconds=queue_length), style="R"),
                        ),
                    ],
    ...
    async def queue(self, interaction: Interaction):
        ...
        embed_list = EmbedGen.EmbedFieldList(
            ...
            fields=[
                EmbedGen.EmbedField(
                    name=getattr(song, "title", "Unknown title"),
                    value=getattr(song, "author", "Unknown author"),
                    inline=False,
                )
    ...
    async def history(self, interaction: Interaction):
        ...
        embed_list = EmbedGen.EmbedFieldList(
            ...
            fields=[
                EmbedGen.EmbedField(
                    name=getattr(song, "title", "Unknown title"), value=getattr(song, "uri", song.source), inline=False
                )
