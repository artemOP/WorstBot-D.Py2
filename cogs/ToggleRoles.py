...
class ToggleRoles(commands.Cog):
    ...
    async def cog_load(self) -> None:
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS toggleroles(id SERIAL PRIMARY KEY, guild BIGINT NOT NULL, role1 BIGINT, role2 BIGINT, UNIQUE(role1, role2))"
        )
    ...
    @app_commands.command(
        name="toggle-role-setup",
        description="Set two roles that can be toggled between, repeating the command will remove the toggle",
    )
    @app_commands.guild_only()
    async def Setup(self, interaction: Interaction, role1: discord.Role, role2: discord.Role):
        await interaction.response.defer(ephemeral=True)
        await self.bot.execute(
            "INSERT INTO toggleroles(guild, role1, role2) VALUES($1, $2, $3) ON CONFLICT(role1, role2) DO UPDATE SET guild = 1",
        ...
        await interaction.followup.send(
            f"{role1.name} <---> {role2.name} Conversion has been added/removed from the setup"
        )
    ...
    async def ToggleRoleAutocomplete(self, interaction: Interaction, current) -> list[app_commands.Choice]:
        toggles = await self.bot.fetch(
            "SELECT id, role1, role2 FROM toggleroles WHERE guild = $1", interaction.guild_id
        )
        return [
            app_commands.Choice(
                name=f"{interaction.guild.get_role(role1).name} <---> {interaction.guild.get_role(role2).name}",
                value=toggle_id,
            )
            for toggle_id, role1, role2 in toggles
        ]
