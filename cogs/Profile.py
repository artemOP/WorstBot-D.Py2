...
class ProfileFill(Modal, title="Profile field"):
    def __init__(self, *, label, name=None, value=None):
        ...
        self.label = label
        self.name = TextInput(
            label="field name", placeholder="enter the title of the field", default=name, required=True, max_length=256
        )
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        embed = FullEmbed(
            title=f"Profile Field {self.label}", fields=[EmbedField(name=self.name.value, value=self.value.value)]
        )
...
class ButtonCallback(Button):
    ...
    async def callback(self, interaction: Interaction) -> None:
        row = await interaction.client.fetchrow(
            "SELECT name, value FROM profile WHERE member = $1 AND index = $2", interaction.user.id, int(self.label)
        )
        if row:
...
class Profile(commands.GroupCog, name="profile"):
    ...
    def __init__(self, bot: WorstBot):
        ...
        self.ContextMenu = app_commands.ContextMenu(name="Profile", callback=self.ProfileContextMenu)
        self.bot.tree.add_command(self.ContextMenu)

    async def cog_load(self) -> None:
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS profile(member BIGINT NOT NULL, name VARCHAR(256), value VARCHAR(1024), index SMALLINT, UNIQUE(member, index))"
        )
    ...
    async def FetchProfile(self, user: int) -> discord.Embed:
        ...
        embed = FullEmbed(
            title="Profile",
            fields=[EmbedField(index=row["index"], name=row["name"], value=row["value"]) for row in rows],
        )
        return embed
