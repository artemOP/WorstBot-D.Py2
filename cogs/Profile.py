...
class ProfileFill(Modal, title="Profile field"):
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        embed = discord.Embed(
            colour=discord.Colour.random(), title=f"Profile Field {self.label}"
        )
...
class ButtonCallback(Button):
    ...
    async def callback(self, interaction: Interaction) -> None:
        row = await interaction.client.fetchrow(
            "SELECT name, value FROM profile WHERE member = $1 AND index = $2",
            interaction.user.id,
            int(self.label),
        )
        if row:
        ...
        await interaction.response.send_modal(
            ProfileFill(label=self.label, name=name, value=value)
        )
...
class Profile(commands.GroupCog, name="profile"):
    def __init__(self, bot: commands.Bot):
        ...
        self.ContextMenu = app_commands.ContextMenu(
            name="Profile", callback=self.ProfileContextMenu
        )
    ...
    async def FetchProfile(self, user: int) -> discord.Embed:
        rows = await self.bot.fetch(
            "SELECT name, value, index FROM profile WHERE member = $1", user
        )
