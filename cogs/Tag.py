...
class TagModal(ui.Modal, title="tag"):
    def __init__(
        ...
        self.MetaData = ui.Select(
            ...
            options=[
                SelectOption(
                    label="NSFW", description="Marks tag as NSFW", default=nsfw
                ),
                SelectOption(
                    label="Private",
                    description="Only allows tag to be used by owner",
                    default=private,
                ),
                SelectOption(
                    label="Invisible", description="Hides tag user", default=invisible
                ),
                SelectOption(
                    label="Public",
                    description="Allows anyone to edit tag",
                    default=public,
                ),
            ],
        )
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        await interaction.followup.send(
            f"{self.TagName.value} has been created", ephemeral=True
        )
...
class Tag(commands.GroupCog, name="tag"):
    ...
    async def OwnerCheck(self, tagid: int, user: int) -> bool:
        select = await self.bot.fetchrow(
            "SELECT owner, public  FROM tags WHERE tagid = $1", tagid
        )
        owner, public = select.values()
        return True if owner == user or public else False

    @app_commands.command(
        name="create", description="create a tag to recall later by name"
    )
    ...
    async def Edit(self, interaction: Interaction, tag: str):
        ...
        select = await self.bot.fetchrow(
            "SELECT name, value, nsfw, private, public, invisible FROM tags WHERE tagid = $1",
            tag,
        )
        if not select:
            return
        ...
        await interaction.response.send_modal(
            TagModal(tag, name, value, nsfw, private, public, invisible)
        )
    ...
    async def Rename(self, interaction: Interaction, tag: str, new_name: str):
        ...
        await self.bot.execute(
            "UPDATE tags SET name = $2 WHERE tagid = $1", tag, new_name
        )
        await interaction.response.send_message(
            f"{name} has been renamed to {new_name}", ephemeral=True
        )
    ...
    async def Delete(self, interaction: Interaction, tag: str):
        ...
        name = await self.bot.execute(
            "DELETE FROM tags WHERE tagid = $1 RETURNING name", tag
        )
        await interaction.response.send_message(
            f"{name} has been Deleted", ephemeral=True
        )
    ...
    async def ViewHelper(self, tag: str, interaction: Interaction, raw: bool):
        ...
        tag = await self.bot.fetchrow(
            "SELECT owner, name, value, nsfw, private, invisible FROM tags WHERE tagid = $1",
            tag,
        )
        ...
        embed = discord.Embed(
            title=tag["name"],
            description=tag["value"] if not raw else f"```txt\n{tag['value']}\n```",
    ...
    async def List(self, interaction: Interaction, user: discord.Member = None):
        ...
        tags = await self.bot.fetch(
            "SELECT name FROM tags WHERE guild = $1 AND owner = $2 AND private = FALSE",
            interaction.guild_id,
            user.id,
        )
        embed = discord.Embed(
            title=f"{user.name}'s tags",
            description="\n".join(tag["name"] for tag in tags)[:4000],
            colour=discord.Colour.random(),
        )
    ...
    async def ListAll(self, interaction: Interaction):
        tags = await self.bot.fetch(
            "SELECT name FROM tags WHERE guild = $1 AND private = FALSE",
            interaction.guild_id,
        )
        embed = discord.Embed(
            title="Server tags",
    ...
    @app_commands.command(
        name="transfer", description="Transfer ownership of tag to someone else"
    )
    async def Transfer(self, interaction: Interaction, tag: str, user: discord.Member):
        if not (
            (tag := await self.bot.to_int(tag))
            or await self.OwnerCheck(tag, interaction.user.id)
        ):
            return
        name = await self.bot.fetchval(
            "UPDATE tags SET owner = $1 WHERE tagid = $2 RETURNING name", user.id, tag
        )
        await interaction.response.send_message(
            f"{user.mention} has become the owner of tag {name}"
        )
    ...
    async def Claim(self, interaction: Interaction, tag: str):
        ...
        owner = self.bot.get_user(
            await self.bot.fetchval("SELECT owner FROM tags WHERE tagid = $1", tag)
        )
        if owner not in interaction.guild.members:
            await self.bot.execute(
                "UPDATE tags SET owner = $1 WHERE tagid = $2", interaction.user.id, tag
            )
            return await interaction.response.send_message(
                "You have successfully claimed the tag", ephemeral=True
            )
        await interaction.response.send_message(
            "cannot claim tag from a user still in the server", ephemeral=True
        )
    ...
    async def PrivilegedTagAutocomplete(
        ...
        return [
            app_commands.Choice(name=name, value=str(tagid)) for tagid, name in tags
        ]

    @View.autocomplete("tag")
    @ViewRaw.autocomplete("tag")
    @Claim.autocomplete("tag")
    async def TagViewAutocomplete(
        self, interaction: Interaction, current: None | str
    ) -> list[app_commands.Choice]:
        current = self.bot.current(current)
        tags = await self.bot.fetch(
            "SELECT tagid, name FROM tags WHERE guild = $1 AND name like $2",
            interaction.guild_id,
            current,
        )
        return [
