...
class Responses(ui.Select):
    ...
    async def callback(self, interaction: Interaction) -> None:
        await interaction.client.execute(
            "DELETE FROM voters WHERE vote_id = $1 AND member = $2", self.vote_id, interaction.user.id
        )
        for answer in self.values:
            await interaction.client.execute(
                "INSERT INTO voters(vote_id, answer_id, member) VALUES($1, $2, $3)",
                self.vote_id,
                int(answer),
                interaction.user.id,
            )
...
class StartPollModal(ui.Modal, title="Poll"):
    def __init__(
        self, *, vote_id: int = None, question: str = None, answers: str = None, accepted_responses: int = None
    ):
        super().__init__()
        self.vote_id = vote_id
        self.question = ui.TextInput(
            label="question", placeholder="enter your question here.", default=question, required=True
        )
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        if self.vote_id:
            await interaction.client.execute(
                "UPDATE votes SET question=$1 WHERE vote_id=$2", self.question.value, self.vote_id
            )
            await interaction.client.execute("DELETE FROM answers WHERE vote_id=$1", self.vote_id)
        else:
            self.vote_id = await interaction.client.fetchval(
                "INSERT INTO votes(guild, question, author, accepted_responses) VALUES($1, $2, $3, $4) ON CONFLICT DO NOTHING RETURNING vote_id",
        ...
        for answer in answers:
            answer = answer.strip()
            answer_id = await interaction.client.execute(
                "INSERT INTO answers(vote_id, answer) VALUES($1, $2) ON CONFLICT(vote_id, answer) DO NOTHING RETURNING answer_id",
                self.vote_id,
                answer,
            )
            if not answer_id:
        ...
        message: discord.Message = await interaction.followup.send(
            view=RespondPollView(vote_id=self.vote_id, options=options, accepted_responses=accepted_responses),
            embed=embed,
        )
        await interaction.client.execute(
            "UPDATE votes set channel=$1, message_id=$2 WHERE vote_id=$3", message.channel.id, message.id, self.vote_id
        )
    ...
    async def detailed_poll_response(self, interaction: Interaction, select: ui.Select):
        await interaction.response.send_message(
            embeds=EmbedGen.SimpleEmbedList(
                descriptions="\n".join(str(member) for member in self.responses[select.values[0]]) or "No votes"
            ),
            ephemeral=True,
        )
...
class Poll(commands.GroupCog, name="poll"):
    ...
    async def cog_load(self) -> None:
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS votes(vote_id SERIAL PRIMARY KEY, guild BIGINT, channel BIGINT DEFAULT NULL, message_id BIGINT DEFAULT NULL , question TEXT NOT NULL, author BIGINT, accepted_responses INT)"
        )
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS answers(vote_id INT REFERENCES votes(vote_id) ON UPDATE CASCADE ON DELETE CASCADE, answer_id SERIAL PRIMARY KEY, answer TEXT NOT NULL, UNIQUE(vote_id, answer))"
        )
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS voters(vote_id INT REFERENCES votes(vote_id) ON UPDATE CASCADE ON DELETE CASCADE, answer_id INT REFERENCES answers(answer_id) ON UPDATE CASCADE ON DELETE CASCADE, member BIGINT NOT NULL)"
        )
        ...
        for poll in polls:
            ...
            self.views.append(
                RespondPollView(
                    ...
                    options=[
                        discord.SelectOption(label=answer["answer"], value=answer["answer_id"]) for answer in answers
                    ],
    ...
    @app_commands.command(
        name="edit", description="Edit a currently active poll. WARNING, this will clear previous responses"
    )
    async def VoteEdit(self, interaction: Interaction, poll_id: int):
        if not await self.bot.fetchval(
            "SELECT EXISTS(SELECT 1 FROM votes WHERE author=$1 AND vote_id=$2)", interaction.user.id, poll_id
        ):
            await interaction.response.send_message("This poll does not belong to you", ephemeral=True)
            return
        poll = await self.bot.fetchrow(
            "SELECT question, channel, message_id, accepted_responses from votes WHERE vote_id=$1", poll_id
        )
    ...
    async def results(self, interaction: Interaction, poll_id: int, ephemeral: bool):
        ...
        poll = await self.bot.fetchrow(
            "SELECT vote_id, message_id, channel, question, author FROM votes WHERE vote_id = $1", poll_id
        )
        answer_table = {
            answer_id: answer
            for answer_id, answer in await self.bot.fetch(
                "SELECT answer_id, answer FROM answers WHERE vote_id=$1", poll_id
            )
        }
        ...
        for answer_id, answer in answer_table.items():
            user_list = (
                await self.bot.fetchval("SELECT ARRAY_AGG(member) FROM voters WHERE answer_id=$1", answer_id) or []
            )
            responses[answer] = [
                await self.bot.maybe_fetch_member(interaction.guild, member) or "<MISSING MEMBER>"
        ...
        embed = EmbedGen.FullEmbed(
            author={
                ...
                "icon_url": None if not author else author.display_avatar.url,
            },
            title=poll["question"],
        ...
        await interaction.followup.send(view=view, embed=embed, file=discord.File(fp=chartIO[1], filename="image.png"))
        chartIO[1].seek(0)
    ...
    async def VoteEnd(self, interaction: Interaction, poll_id: int):
        ...
        if interaction.user.id != owner:
            return await interaction.response.send_message(
                f"This is not your poll to end, please use {Converters.to_command_mention(self.VoteResults, interaction.guild)}",
                ephemeral=True,
            )
        await self.results(interaction, poll_id, ephemeral=False)
        channel, message = await self.bot.fetchrow(
            "DELETE FROM votes WHERE vote_id = $1 RETURNING channel, message_id", poll_id
        )  # type: int, int
    ...
    async def ResponsePollAutocomplete(self, interaction: Interaction, current):
        ...
        responses = await self.bot.fetch(
            "SELECT vote_id, question FROM votes WHERE guild=$1 AND question LIKE $2", interaction.guild_id, current
        )
