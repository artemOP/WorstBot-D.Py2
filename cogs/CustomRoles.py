...
class hexTransformer(app_commands.Transformer):
    async def autocomplete(
        ...
        return [
            Choice(name="abcdef", value="abcdef"),
            Choice(name="0x012345", value="0x012345"),
            Choice(name="#1120ff", value="#1120ff"),
        ]
...
class CustomRoles(commands.GroupCog, name="role"):
    def __init__(self, bot: WorstBot):
        super().__init__()
        self.bot = bot
        self.ContextMenu = app_commands.ContextMenu(name="Colour Check", callback=self.colourCheck)
        self.bot.tree.add_command(self.ContextMenu)
        self.logger = self.bot.logger.getChild(self.qualified_name)

    async def cog_load(self) -> None:
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS customroles(guild BIGINT, member BIGINT, role BIGINT UNIQUE, colour INT, UNIQUE(guild, member))"
        )
    ...
    async def FetchRole(self, *, guild: discord.Guild, member: discord.Member) -> Optional[discord.Role]:
        ...
        if (
            roleid := await self.bot.fetchval(
                "SELECT role FROM customroles WHERE guild = $1 AND member = $2", guild.id, member.id
            )
        ) is not None:
    ...
    async def CreateRole(self, member: discord.Member, colour: int) -> None:
        ...
        await self.bot.execute(
            "INSERT INTO customroles(guild, member, role, colour) VALUES ($1, $2, $3, $4) ON CONFLICT (guild, member) DO UPDATE SET role = EXCLUDED.role, colour = EXCLUDED.colour",
            member.guild.id,
            member.id,
            custom_role.id,
            colour,
    ...
    async def on_member_join(self, member):
        ...
        if (
            colour := await self.bot.fetchval(
                "SELECT colour FROM customroles WHERE guild = $1 AND member = $2", member.guild.id, member.id
            )
        ) is None:
    ...
    async def on_member_update(self, before, after):
        ...
        role_id = await self.bot.fetchval(
            "SELECT role FROM customroles WHERE guild = $1 AND member = $2", before.guild.id, before.id
        )
        ...
        if role is None:
            ...
            await interaction.followup.send(
                embed=EmbedGen.SimpleEmbed(
                    colour=role.colour, text=f"{member.name} Uses the role colour {role.colour}"
                ),
                ephemeral=True,
            )
