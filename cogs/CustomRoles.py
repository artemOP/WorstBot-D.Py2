...
class CustomRoles(commands.GroupCog, name="role"):
    def __init__(self, bot: commands.Bot):
        ...
        self.ContextMenu = app_commands.ContextMenu(
            name="Colour Check", callback=self.colourCheck
        )
    ...
    async def on_ready(self):
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS customroles(guild BIGINT, member BIGINT, role BIGINT UNIQUE, colour INT)"
        )
    ...
    async def FetchRole(
        ...
        if (
            roleid := await self.bot.fetchval(
                "SELECT role FROM customroles WHERE guild = $1 AND member = $2",
                guild.id,
                member.id,
            )
        ) is not None:
    ...
    async def CreateRole(self, member: discord.Member, colour: int) -> None:
        role = await member.guild.create_role(
            name=str(member), colour=discord.Colour(colour), hoist=False
        )
        position = member.top_role.position if member.top_role.position > 0 else 1
        await role.edit(position=position)
        await member.add_roles(role)
        await self.bot.execute(
            "INSERT INTO customroles(guild, member, role, colour) VALUES ($1, $2, $3, $4) ON CONFLICT (role) DO UPDATE SET role = EXCLUDED.role",
            member.guild.id,
            member.id,
            role.id,
            colour,
    ...
    async def on_member_join(self, member):
        if (
            await self.bot.fetchval(
                "SELECT roles FROM events WHERE guild = $1", member.guild.id
            )
            is False
        ):
            return
        if (
            colour := await self.bot.fetchval(
                "SELECT colour FROM customroles WHERE guild = $1 AND member = $2",
                member.guild.id,
                member.id,
            )
        ) is None:
    ...
    async def on_user_update(self, before, after):
        if before.bot or str(before) == str(after):
            return
        role_id = await self.bot.fetchval(
            "SELECT role FROM customroles WHERE guild = $1 AND member = $2",
            before.guild.id,
            before.id,
        )
    ...
    @app_commands.command(
        name="colour", description="edits the colour of your custom role"
    )
    async def EditRole(
        self,
        interaction: Interaction,
        colour: app_commands.Transform[int, hexTransformer],
    ):
        ...
        await interaction.response.send_message(
            content=f"your role colour has now been set to {hex(colour)}",
            ephemeral=True,
        )

    @app_commands.command(name="check")
    async def colourCheckCommand(
        self, interaction: Interaction, arg: Optional[discord.User] = None
    ):
    ...
    async def colourCheck(self, interaction: Interaction, arg: discord.User = None):
        ...
        if role is None:
            await interaction.followup.send(
                "This user does not have a custom role or it may be broken"
            )
        else:
            embed = discord.Embed(
                colour=role.colour,
                description=f"{member.name} Uses the role colour {role.colour}",
            )
