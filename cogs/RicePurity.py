...
class PurityButtons(discord.ui.View):  # Makes The quiz buttons run and gives output
    ...
    async def on_complete(self, interaction: Interaction):
        ...
        await self.bot.execute(
            "INSERT INTO ricepurity(id, score) VALUES($1, $2) ON CONFLICT (id) DO UPDATE SET score = excluded.score",
            interaction.user.id,
            self.score,
        )
    ...
    async def tick(self, interaction: Interaction, button: Button):
        ...
        if self.counter == 100:
            ...
            await interaction.response.edit_message(
                view=self, content=": ".join(next(self.generator))
            )
...
class PurityLeaderboard(discord.ui.View):
    ...
    @button(
        ...
        custom_id="RicePurityPersistent:FirstPage",
    )
    ...
    @button(
        label="Previous page",
        style=discord.ButtonStyle.red,
        custom_id="RicePurityPersistent:back",
    )
    ...
    @button(
        ...
        custom_id="RicePurityPersistent:exit",
    )
    ...
    @button(
        ...
        custom_id="RicePurityPersistent:forward",
    )
    ...
    @button(
        label="Last Page",
        style=discord.ButtonStyle.green,
        custom_id="RicePurityPersistent:LastPage",
    )
...
class RicePurity(commands.GroupCog, name="ricepurity"):  # Main cog class
    ...
    async def on_ready(self):
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS ricepurity(id BIGINT PRIMARY KEY, score INT)"
        )
    ...
    async def embedforming(self, users):
        ...
        while totalcount < len(users):
            fieldcount = 0
            embed = discord.Embed(
                colour=discord.Colour.dark_purple(), title="Rice Purity Scores"
            )
            while fieldcount < 24 and totalcount < len(users):
                embed.add_field(
                    name=self.bot.get_user(list(users.keys())[totalcount]),
                    value=list(users.values())[totalcount],
                )
                fieldcount += 1
                totalcount += 1
            embed.set_footer(
                text=f"Page {ceil(totalcount / 25)} of {ceil(len(users) / 25)}"
            )
    ...
    async def test(self, interaction: Interaction):
        ...
        await interaction.response.send_message(
            "Are you ready to begin your rice purity test?", view=view, ephemeral=True
        )
    ...
    async def leaderboard(self, interaction: Interaction):
        ...
        for member in interaction.guild.members:
            if (
                score := await self.bot.fetchval(
                    "SELECT score FROM ricepurity WHERE id=$1", member.id
                )
            ) is not None:
                users[member.id] = score
        users = {
            key: value for key, value in sorted(users.items(), key=lambda item: item[1])
        }
