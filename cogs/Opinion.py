...
class Opinion(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if (
            await self.bot.fetchval(
                "SELECT opinion FROM events WHERE guild = $1", message.guild.id
            )
            is False
        ):
            return
        prefixes = await self.bot.fetch(
            "SELECT prefix FROM PrefixBlacklist WHERE guild = $1", message.guild.id
        )
        ...
        content = sub(
            r"<(?P<animated>a?):(?P<name>\w{2,32}):(?P<id>\d{18,22})>", "", content
        )
        await self.bot.execute(
    ...
    @app_commands.command(
        name="opinion",
        description="Ask worst bot for its opinion on your super important questions",
    )
    async def opinion(self, interaction: discord.Interaction, opinion: str = None):
        row = await self.bot.fetchrow(
            "SELECT content FROM Opinion WHERE GUILD=$1 ORDER BY random() LIMIT 1",
    ...
    @app_commands.command(
        ...
        description="blacklist prefixes used by other bots in opinion forming",
    )
    @app_commands.default_permissions()
    async def PrefixBlacklist(self, interaction: discord.Interaction, prefix: str):
        await self.bot.execute(
            "INSERT INTO prefixblacklist(guild, prefix) VALUES ($1, $2)",
            interaction.guild_id,
            prefix,
        )
        await interaction.response.send_message(
            f"messages starting with {prefix} will be ignored", ephemeral=True
        )

    @tasks.loop(hours=24, reconnect=True)
    async def DeleteOld(self):
        await self.bot.execute(
            "DELETE FROM opinion WHERE timestamp < CURRENT_TIMESTAMP - INTERVAL '2 weeks'"
        )
