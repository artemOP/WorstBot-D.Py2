...
class Opinion(commands.Cog):
    ...
    async def on_ready(self):
        await self.bot.execute("CREATE TABLE IF NOT EXISTS Opinion(guild BIGINT NOT NULL, timestamp TIMESTAMP WITH TIME ZONE NOT NULL, content TEXT DEFAULT NULL, attachment TEXT DEFAULT NULL)")
        ...
        if message.author.bot is False and message.channel.is_nsfw() is False:
            if not any(message.content.startswith(prefix) for prefix in self.prefixes):
                if message.attachments:
                    ...
                    message.attachments = ""
                await self.bot.execute("INSERT INTO Opinion(guild, timestamp, content, attachment) VALUES ($1,$2,$3,$4)", message.guild.id, message.created_at, message.clean_content, message.attachments)
