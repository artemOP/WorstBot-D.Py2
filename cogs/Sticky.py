...
class StickyMessage(commands.GroupCog, name="sticky"):
    ...
    @app_commands.command(
        name="add", description="Pin a message to the bottom of a channel"
    )
    ...
    async def StickyRemove(self, interaction: discord.Interaction):
        await self.bot.execute(
            "DELETE FROM sticky WHERE channel=$1", interaction.channel_id
        )
        await interaction.response.send_message(
            "Sticky has been removed from this channel."
        )

    @commands.Cog.listener()
    async def on_message(self, message):
        sticky = await self.bot.fetchval(
            "SELECT EXISTS( SELECT 1 FROM sticky WHERE channel = $1)",
            message.channel.id,
        )
        if message.author.bot or not sticky:
            return
        sticky = await self.bot.fetchrow(
            "SELECT * FROM sticky WHERE channel=$1", message.channel.id
        )
        if sticky["messageid"]:
            try:
                oldmessage = await message.channel.fetch_message(sticky["messageid"])
            except:
                oldmessage = await message.channel.fetch_message(
                    message.channel.last_message_id
                )
            await oldmessage.delete()
        sticky = await message.channel.send(sticky["message"])
        await self.bot.execute(
            "UPDATE sticky SET messageid=$1 WHERE channel=$2",
            sticky.id,
            message.channel.id,
        )
