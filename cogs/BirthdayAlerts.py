...
class BirthdayAlert(commands.GroupCog, name="birthday"):
    ...
    async def on_ready(self):
        ...
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS birthdays(guild BIGINT, member BIGINT, birthday DATE, PRIMARY KEY(guild, member))"
        )
    ...
    async def BirthdayAdd(
        ...
        month: Range[int, 1, 12] = None,
        day: Range[int, 1, 31] = None,
    ):
        if not (month or day):
            await self.bot.execute(
                "DELETE FROM birthdays WHERE member = $1", interaction.user.id
            )
            return await interaction.response.send_message(
                f"Birthday removed from bot", ephemeral=True
            )
        birthday = date(year=date.today().year, month=month, day=day)
        if birthday < date.today():
            birthday += relativedelta(years=+1)
        await self.bot.execute(
            "INSERT INTO birthdays(guild, member, birthday) VALUES($1, $2, $3) ON CONFLICT(guild, member) DO UPDATE SET birthday = excluded.birthday",
            interaction.guild_id,
            interaction.user.id,
            birthday,
        )
        await interaction.response.send_message(
            f"you will be alerted of your birthday on {birthday.strftime('%d/%m/%Y')}",
            ephemeral=True,
        )

    @app_commands.command(name="list")
    async def BirthdayList(self, interaction: Interaction):
        table: list[Record] = await self.bot.fetch(
            "SELECT member, birthdays FROM birthdays WHERE guild = $1",
            interaction.guild_id,
        )
        birthdays: dict[discord.Member : date] = {
            interaction.guild.get_member(member): date.today() + interval
            for member, interval in table
        }
        embedlist: list[discord.Embed] = await self.EmbedFormer(birthdays)
        view = BirthdayView(timeout=30, embedlist=embedlist)
        await interaction.response.send_message(
            view=view, embed=embedlist[0], ephemeral=True
        )
        view.response = await interaction.original_message()

    @tasks.loop(time=time(1, 0), reconnect=True)
    async def BirthdayCheck(self):
        birthdays = await self.bot.fetch(
            "SELECT * FROM birthdays WHERE birthday = NOW()::DATE"
        )
        if not birthdays:
            return
        print(birthdays)
        for birthday in birthdays:
            if await self.bot.fetchval(
                "SELECT birthdays FROM events WHERE guild = $1", birthday["guild"]
            ):
                return
            guild = self.bot.get_guild(birthday["guild"])
            if not guild:
                return await self.bot.execute(
                    "DELETE FROM birthdays WHERE guild = $1", birthday["guild"]
                )
            channel = await self.bot.fetchval(
                "SELECT channel FROM birthdaychannel WHERE guild = $1", guild.id
            )
            channel = guild.get_channel(channel)
            member = guild.get_member(birthday["member"])
            await channel.send(
                f"Today is {member.mention}'s birthday, dont forget to send them a happy birthday message"
            )
            await self.bot.execute(
                "UPDATE birthdays SET birthday = birthday + INTERVAL '1 year' WHERE member = $1",
                member.id,
            )
