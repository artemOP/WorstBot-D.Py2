...
class Stats(commands.GroupCog, name="stats"):
    ...
    async def cog_load(self) -> None:
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS usage(command TEXT, guild BIGINT, execution_time timestamptz)"
        )
    ...
    async def github(self, interaction: Interaction):
        ...
        embed_list = EmbedGen.EmbedFieldList(
            ...
            fields=[
                ...
                for week in data
                if not (week["a"] == 0 and week["d"] == 0 and week["c"] == 0)
            ],
            max_fields=9,
        )
        ...
        await interaction.followup.send(
            view=view,
            embed=embed_list[0],
            content=f"Total commits: {contributors.get('data').get('total')}",
            ephemeral=True,
        )
        view.response = await interaction.original_response()

    @app_commands.command(name="server-usage")
    async def GuildUsage(
        self,
        interaction: Interaction,
        before: Transform[str, DateTransformer] = Converters.to_datetime("2100/01/01", "%Y/%m/%d"),
        after: Transform[str, DateTransformer] = Converters.to_datetime("1970/01/01", "%Y/%m/%d"),
    ):
        ...
        usage = await self.bot.fetch(
            "SELECT command, COUNT(*) AS count FROM usage WHERE guild = $1 AND execution_time BETWEEN $2::TIMESTAMP AND $3::TIMESTAMP GROUP BY command ORDER BY count DESC",
            interaction.guild_id,
            after,
            before,
        )
        total = sum(row["count"] for row in usage)
        chartIO = await Graphs.graph(
            "pie", self.bot.loop, {row["command"]: row["count"] for row in usage if (row["count"] / total) * 100 > 3}
        )
        view = Paginators.ThemedGraphView({"Light": chartIO[0], "Dark": chartIO[1]})
        embeds = EmbedGen.SimpleEmbedList(
            title="Guild command usage",
            descriptions="\n".join(f"{row['command']}: {row['count']}" for row in usage),
            image="attachment://image.png",
            footer={"text": f"Total uses: {total}"},
        )
        await interaction.followup.send(
            view=view, embeds=embeds, file=discord.File(fp=chartIO[1], filename="image.png")
        )
        view.response = await interaction.original_response()
    ...
    async def GloablUsage(self, interaction: Interaction):
        await interaction.response.defer(ephemeral=True)
        usage = await self.bot.fetch(
            "SELECT command, COUNT(*) as count, max(execution_time) as last_usage FROM usage GROUP BY command ORDER BY count DESC"
        )
        total = sum(row["count"] for row in usage)
        chartIO = await Graphs.graph(
            "pie", self.bot.loop, {row["command"]: row["count"] for row in usage if (row["count"] / total) * 100 > 3}
    ...
    async def line_count(self, interaction: Interaction):
        ...
        for key, file in self.line_count.items():  # type: str, File
            fields.append(EmbedGen.EmbedField(name=key, value=await self.text_formatting(file)))
        ...
        embed_list = EmbedGen.EmbedFieldList(author=author, title="Line Count", fields=fields, max_fields=9)
        embed_list.insert(
            0, EmbedGen.SimpleEmbed(title="Project total", author=author, text=await self.text_formatting(total_lines))
        )
    ...
    async def CommandUsage(
        ...
        if await self.bot.events(interaction.guild_id, self.bot._events.usage) is False:
            return
        await self.bot.execute(
            "INSERT INTO usage(command, guild, execution_time) VALUES($1, $2, $3)",
