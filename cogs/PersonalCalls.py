...
class PersonalCalls(commands.GroupCog, name="personal-call"):
    ...
    async def on_voice_state_update(
        ...
        if after.channel == base_call:
            ...
            personal_call = await base_call.category.create_voice_channel(
    ...
    async def CallBlacklistToggle(self, interaction: Interaction, channel: discord.VoiceChannel):
        ...
        if not await self.bot.fetchval(
            ...
            await self.bot.execute(
                "INSERT INTO CallBlacklist(guild, channel) VALUES($1, $2)", interaction.guild_id, channel.id
            )
    ...
    async def CallBlacklistList(self, interaction: Interaction):
        ...
        channels = await self.bot.fetch(
            "SELECT channel FROM CallBlacklist WHERE guild=$1 LIMIT 25", interaction.guild.id
        )
        embed = EmbedGen.FullEmbed(
            title="Protected channels",
            fields=[
                EmbedGen.EmbedField(
                    name=str(await self.bot.maybe_fetch_channel(channel["channel"])), value=Constants.BLANK
                )
    ...
    async def UserBlacklist(self, interaction: Interaction, member: discord.Member):
        ...
        if not await self.bot.fetchval(
            ...
            await self.bot.execute(
                "DELETE FROM UserBlacklist WHERE guild = $1 AND member = $2", interaction.guild_id, member.id
            )
