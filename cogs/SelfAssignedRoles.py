...
class RoleTransformer(Transformer):
    ...
    async def autocomplete(cls, interaction: Interaction, current) -> list[Choice[str]]:
        roleids = await interaction.client.fetch(
            "SELECT role FROM selfroles WHERE guild = $1", interaction.guild_id
        )
        roles = {
            role["role"]: interaction.guild.get_role(role["role"]).name
            for role in roleids
        }
        if not current:
            return [Choice(name=item[1], value=str(item[0])) for item in roles.items()]
        return [
            Choice(name=item[1], value=str(item[0]))
...
class SelfAssignableRoles(
    commands.GroupCog, name="giveme", description="Toggle a self assignable role"
):
    ...
    async def on_ready(self):
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS selfroles(guild BIGINT NOT NULL, role BIGINT PRIMARY KEY)"
        )
        print("SelfAssignableRoles Cog online")

    @app_commands.command(name="role")
    async def ToggleRole(
        self, interaction: Interaction, role: Transform[discord.Role, RoleTransformer]
    ):
        if not role:
        ...
        if role in interaction.user.roles:
            ...
            await interaction.response.send_message(
                f"You have removed the {role.name} role", ephemeral=True
            )
        else:
            await interaction.user.add_roles(role)
            await interaction.response.send_message(
                f"You have added the {role.name} role", ephemeral=True
            )
