...
class Twitch(commands.GroupCog, name="twitch"):
    ...
    async def on_ready(self):
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS twitch(guild BIGINT NOT NULL, channel BIGINT NOT NULL, userid BIGINT NOT NULL, live BOOLEAN NOT NULL DEFAULT FALSE, UNIQUE(guild, userid))"
        )
        await self.TokenGen()
    ...
    @app_commands.command(
        name="add", description="Get live alerts for your selected twitch channel"
    )
    async def LiveTrackingAdd(
        self, interaction: Interaction, channel: discord.TextChannel, twitch_user: str
    ):
        twitch_user = await self.bot.to_int(twitch_user)
        await self.bot.execute(
            "INSERT INTO twitch(guild, channel, userid) VALUES($1, $2, $3) ON CONFLICT (guild, userid) DO NOTHING",
        ...
        await interaction.response.send_message(
            "Streamer has been added to the Tracking list", ephemeral=True
        )

    @app_commands.command(
        name="remove", description="Remove live alerts from your selected channel"
    )
    async def LiveTrackingRemove(self, interaction: Interaction, twitch_user: str):
        ...
        await self.streamers()
        await interaction.response.send_message(
            "Streamer has been removed from the Tracking list", ephemeral=True
        )
    ...
    async def LiveTrackingAddAutocomplete(self, interaction: Interaction, current):
        ...
        return [
            app_commands.Choice(name=response["display_name"], value=response["id"])
    ...
    async def LiveTrackingRemoveAutocomplete(self, interaction: Interaction, current):
        if not await self.validate(self.token):
            return []
        streamIDs = await self.bot.fetch(
            "SELECT userid FROM twitch WHERE guild=$1 LIMIT 25", interaction.guild_id
        )
        if not streamIDs:
            return []
        ...
        return [
            app_commands.Choice(
                name=streamer["broadcaster_name"], value=streamer["broadcaster_id"]
            )
        ...
        for user in self.streamersTable:
            if (
                await self.bot.fetchval(
                    "SELECT twitch FROM events WHERE guild = $1", user["guild"]
                )
                is False
            ):
                continue
            if not any(
                int(stream["user_id"]) == user["userid"] for stream in streams["data"]
            ):
                await self.bot.execute(
                    "UPDATE twitch SET live = FALSE WHERE userid = $1", user["userid"]
                )
                continue
            if (
                await self.bot.fetchval(
                    "SELECT live FROM twitch WHERE userid=$1", user["userid"]
                )
                is True
            ):
                continue
            ...
            await self.bot.execute(
                "UPDATE twitch SET live = TRUE WHERE userid=$1", user["userid"]
            )
            stream = [
                dictionary
                for dictionary in streams["data"]
                if int(dictionary["user_id"]) == user["userid"]
            ][0]
            channel = self.bot.get_channel(user["channel"])
            embed = discord.Embed(
                colour=discord.Colour.random(), title=stream["user_name"]
            )
