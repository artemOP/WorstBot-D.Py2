    ...
    async def start(self, interaction: Interaction):
        ...
        match self.mode:
            case Games.blackjack:
                self.cards: deque[Emoji] = deque(
                    choices([emoji for emoji in self.bot.custom_emoji if emoji.name.startswith("card_")], k=52)
                )
    ...
    async def cancel(self, interaction: Interaction):
        ...
        for player in self.players:
            await player.view.response.edit(
                view=None, embed=None, content="The game has been cancelled by the host", delete_after=30
            )
...
class Blackjack(Game):
    ...
    async def game_end(self, interaction: Interaction):
        ...
        winners = [
            player
            for player in self.game.players
            if player.state is PlayerState.sticking
            and sum(Cards[card.name.split("_")[1]].value for card in player.view.cards) > dealer_score
        ]
        ...
        embed = EmbedGen.FullEmbed(
            ...
            fields=[
                EmbedGen.EmbedField(
                    name="Winner",
                    value=interaction.guild.get_member(winner.wealth.member_id).mention if winner else "Dealer wins",
                ),
                EmbedGen.EmbedField(
                    name="Winning Hand",
                    value=(
                        "".join(str(card) for card in winner.view.cards)
                        if winner
                        else "".join(str(card) for card in self.game.dealer)
                    ),
                ),
        ...
        match bet_type:
            ...
            case RouletteBets.lower_straight_up:
                self.bet_value.options = [
                    discord.SelectOption(label=str(number), value=str(number)) for number in range(0, 19)
                ]
                self.bet_value.disabled = False

            case RouletteBets.upper_straight_up:
                self.bet_value.options = [
                    discord.SelectOption(label=str(number), value=str(number)) for number in range(19, 37)
                ]
                self.bet_value.disabled = False

            case RouletteBets.street:
                self.bet_value.options = [
                    discord.SelectOption(label=f"{number} - {number + 2}", value=f"{number}-{number + 2}")
    ...
    async def confirm(self, interaction: Interaction, button: ui.Button):
        ...
        if all(player.state is PlayerState.sticking for player in self.game.players):
            ...
            await interaction.response.edit_message(
                view=None, content="You have placed your bet, please wait for other players"
            )
    ...
    async def game_end(self, interaction: Interaction):
        ...
        for player in self.game.players:
            ...
            if len(bet) == 1 and isinstance(bet[0], int):  # Straight up
                ...
                (bet[0] is RouletteBets.red and self.winning_number[1] is RouletteColours.red)  # Red
                or (bet[0] is RouletteBets.black and self.winning_number[1] is RouletteColours.black)  # Black
                or (bet[0] is RouletteBets.even and self.winning_number[0] % 2 == 0)  # Even
                or (bet[0] is RouletteBets.odd and self.winning_number[0] % 2 == 1)  # Odd
                or (bet[0] is RouletteBets.low and 1 <= self.winning_number[0] <= 18)  # Low
                or (bet[0] is RouletteBets.high and 19 <= self.winning_number[0] <= 36)  # High
            ):
                await reward_winner(player, 2)
            elif (
                (bet[0] is RouletteBets.column_1 and self.winning_number[0] in range(1, 37, 3))  # Column 1
                or (bet[0] is RouletteBets.column_2 and self.winning_number[0] in range(2, 37, 3))  # Column 2
                or (bet[0] is RouletteBets.column_3 and self.winning_number[0] in range(3, 37, 3))  # Column 3
                or (bet[0] is RouletteBets.dozen_1 and 1 <= self.winning_number[0] <= 12)  # Dozen 1
                or (bet[0] is RouletteBets.dozen_2 and 13 <= self.winning_number[0] <= 24)  # Dozen 2
                or (bet[0] is RouletteBets.dozen_3 and 25 <= self.winning_number[0] <= 36)  # Dozen 3
            ):
        ...
        embed = EmbedGen.FullEmbed(
            ...
            fields=[
                EmbedGen.EmbedField(
                    name="Winning combination: ", value=f"{self.winning_number[1].name} {self.winning_number[0]}"
                )
            ],
        )
        for player in self.game.players:
...
class Gambling(commands.GroupCog, name="gambling"):
    ...
    async def start_lobby(self, interaction: Interaction, game: Games, bet: app_commands.Range[int, 100, 100_000]):
        ...
        if game in self.games[interaction.guild] and self.games[interaction.guild][game].state not in (
            GameState.cancelled,
            GameState.finished,
        ):
            return await interaction.response.send_message(
                f"{game.name.title()} lobby already open, join it with {Converters.to_command_mention(self.join_lobby, interaction.guild)}",
                ephemeral=True,
            )
        elif wealth.wallet < bet:
            return await interaction.response.send_message(
                f"You need at least {bet} WorstCoin in your wallet to start a lobby with this bet", ephemeral=True
            )
        ...
        message = await interaction.followup.send(
            f"{interaction.user.mention} has started a {game.mode.name.title()} lobby\nJoin it using {Converters.to_command_mention(self.join_lobby, interaction.guild)}",
            wait=True,
            silent=True,
        )
        await sleep(30)
    ...
    async def join_lobby(self, interaction: Interaction, game: Games, bet: app_commands.Range[int, 100, 100_000]):
        ...
        if game not in self.games[interaction.guild]:
            return await interaction.response.send_message(
                f"{game.name.title()} lobby not open, start one with {Converters.to_command_mention(self.start_lobby, interaction.guild)}",
                ephemeral=True,
            )
        elif self.games[interaction.guild][game].state is (GameState.cancelled, GameState.finished):
            return await interaction.response.send_message(
                f"{game.name.title()} lobby cancelled, start a new one with {Converters.to_command_mention(self.start_lobby, interaction.guild)}",
                ephemeral=True,
            )
        elif self.games[interaction.guild][game].state is GameState.playing:
            return await interaction.response.send_message(
                f"{game.name.title()} lobby already playing, please wait for it to finish", ephemeral=True
            )
        elif wealth.wallet < bet:
            return await interaction.response.send_message(
                f"You need at least {bet} WorstCoin to join a lobby with this bet", ephemeral=True
            )
        ...
        embed = EmbedGen.FullEmbed(
            ...
            fields=[
                EmbedGen.EmbedField(name="Players", value=len(self.games[interaction.guild][game].players), inline=True)
            ],
        )
        await interaction.response.send_message(view=view, embed=embed, ephemeral=True)
        view.response = await interaction.original_response()
