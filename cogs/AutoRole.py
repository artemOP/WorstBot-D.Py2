...
async def embedforming(role):
    ...
    while totalcount < len(role):
        fieldcount = 0
        embed = discord.Embed(
            colour=discord.Colour.dark_purple(), title="Automatically applied roles"
        )
...
class AutoRole(commands.GroupCog, name="autorole"):
    ...
    async def on_ready(self):
        ...
        await self.bot.execute(
            "CREATE TABLE IF NOT EXISTS autorole(guild BIGINT, role BIGINT UNIQUE )"
        )
    ...
    async def AutoRole(self, interaction: Interaction, role: discord.Role):
        if not await self.bot.fetchval(
            "SELECT EXISTS(SELECT 1 FROM autorole WHERE guild = $1 AND role = $2)",
            ...
            await interaction.response.send_message(
                f"{role.name} successfully added to the AutoRole"
            )
        else:
            await self.bot.execute(
                "DELETE FROM autorole WHERE guild = $1 AND role = $2",
                interaction.guild.id,
                role.id,
            )
            await interaction.response.send_message(
                content=f"{role.name} successfully removed to the AutoRole"
            )

    @app_commands.command(name="list")
    async def AutoRoleList(self, interaction: Interaction):
        view = AutoRoleList(timeout=60)
        roles = await self.bot.fetch(
            "SELECT role FROM autorole WHERE guild=$1", interaction.guild.id
        )
        for role in roles:
    ...
    async def on_member_join(self, member):
        if (
            await self.bot.fetchval(
                "SELECT autorole FROM events WHERE guild = $1", member.guild.id
            )
            is False
        ):
            return
        roles = await self.bot.fetch(
            "SELECT role FROM autorole WHERE guild=$1", member.guild.id
        )
        for role in roles:
